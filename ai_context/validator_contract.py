...
class ValidatorIssuePayload(TypedDict):
    ...
    # 1-based line number, or 1 as a neutral placeholder when the issue has no line concept.
    line: int

    # Human-readable description of the issue.
    message: str

    # 'error' (blocks gate), 'warning' (visible but non-blocking), or 'info'.
    severity: str

    # Actionable remediation instruction for this specific issue.
    suggested_fix: str

    # Ordered list of files/paths an agent should read before attempting a fix.
    read_first: list[str]

    # Ordered list of commands to run after applying the fix, to confirm resolution.
    next_commands: list[str]

    # The condition under which the agent should stop expanding the diff — prevents runaway
...
def build_validator_issue_payload(
    *,
    ...
    if playbook is not None:
        # suggested_fix falls back to likely_fix_shape for playbooks that only
        ...
        # Internal playbooks name this key next_checks; a few external call sites
    ...
    # Annotated as the TypedDict rather than dict[str, object] so mypy checks the
    ...
    # Widened before merging extras — a TypedDict is closed, and validators
