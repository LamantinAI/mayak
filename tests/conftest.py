...
class _FixtureSettings(Settings):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()
        # Run tests on the production path. With debug=True, Starlette answers
        ...
        # A concrete origin, because validate_runtime() rejects the wildcard
...
def fastapi_app(test_settings: _FixtureSettings) -> Generator[FastAPI, None, None]:
    ...
    try:
        # Build FastAPI application with test settings.
...
def log_capture() -> Generator[list, None, None]:
    ...
    # Mock the underlying logger methods.
