...
class TestSerialization:
    ...
    def test_safe_serialize_redacts_every_declared_key(self, key: str) -> None:
        # Parametrised over the constant, so a key ADDED to the set is exercised
    ...
    def test_every_key_named_by_hand_is_still_redacted(self, key: str) -> None:
        # Runs the real function over a literal name, not a membership check on
    ...
    def test_redaction_ignores_case_and_hyphens(self, key: str) -> None:
        # Every other test in this class spells its keys in lower case, so the
    ...
    def test_safe_serialize_exempts_every_declared_exempt_key(self, key: str) -> None:
        # Parametrised over the constant, the mirror of the REDACT_KEYS test above.
