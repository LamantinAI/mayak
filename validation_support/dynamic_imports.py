...
# The module whose import functions this detector follows.
_IMPORTLIB_MODULE = "importlib"

# Functions in importlib whose first argument names a module to import.
_IMPORT_FUNCTIONS = frozenset({"import_module", "__import__"})

# The keyword both functions accept for the module name, e.g. `__import__(name="psycopg")`.
_MODULE_ARGUMENT = "name"


# The names one file has bound to importlib and to its import functions.
@dataclass
class _Bindings:
    # Names that refer to the importlib module here — `importlib`, plus any `as` alias.
    modules: set[str] = field(default_factory=set)

    # Bare names that call an import function — `__import__` always, plus whatever
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                # `import importlib.util` binds the name `importlib` too, so the
...
def dynamic_import_targets(tree: ast.AST) -> list[tuple[str, int]]:
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(func, ast.Attribute):
            # The receiver has to be a name this file bound to importlib. Without
