...
# Reads .gitignore file from the root directory and combines it with defaults.
def load_gitignore_patterns(root_path: Path) -> list:
    # Load patterns from .gitignore and merge with default exclusions.
...
# Resolve the project's display name for the map header. Path.cwd().name is forbidden as a
...
# RuntimeError: If neither source yields a usable name.
def resolve_project_name(root_path: Path) -> str:
    # Primary source — docs/project_context.json:project_name (schema-validated by
    ...
    # Fallback source — pyproject.toml's [project].name. tomllib is stdlib from
...
# Build a subprocess environment with every inherited GIT_*-prefixed variable stripped, so
...
# Recursive function to traverse real directories and build a tree with descriptions.
...
def generate_tree(
    ...
    # Recursively traverse directories and build tree with descriptions.
...
# Render a tree from the git-index-derived virtual structure (build_tree_from_paths),
...
def _compute_updated_content(file_path: Path, tree_content: str) -> str:
    ...
    # Verify the target file exists before attempting marker replacement.
    if not file_path.exists():
        raise FileNotFoundError(str(file_path))

    # Read target content and locate the project map markers.
    ...
    # Replace the existing project map block with the newly generated tree.
    return (
...
def _build_tree_content(root_dir: Path) -> str:
    ...
    sys.stdout = buffer
    try:
        # The two-line preamble is generated rather than typed above the marker.
...
def main() -> int:
    ...
    if args.check:
        # Drift-check mode — bail out on a missing target before paying the cost of
    ...
    if args.check:
        ...
        # Drift detected — print actionable header + unified diff to stderr.
        sys.stderr.write(
    ...
    # Default mode — write the regenerated tree into docs/project_map.md if present.
