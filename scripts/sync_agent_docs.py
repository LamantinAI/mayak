...
def _render_import_stub() -> str:
    ...
    return (
        ...
        f"one rendered contract instead of\nrepeating it. The import must stay a bare line: inside "
        "backticks or a fenced block it is text rather\nthan an import, and the session then starts "
        "with no rules at all. See docs/adr/ADR-011-one-rendered-wrapper.md.\n"
        "-->\n"
        f"{IMPORT_LINE}\n"
    )
