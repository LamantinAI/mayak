...
# One rendered contract under two names. Codex reads AGENTS.md and Claude Code reads CLAUDE.md;
# neither reads the other's file. So AGENTS.md carries the rules and CLAUDE.md carries an import of
# it, rather than a second copy of the same twelve hundred words that a gate then has to keep equal
# — ADR-011. An import and not a symlink: git on Windows without Developer Mode checks a symlink out
# as a text file holding the target's name, and there the rules would reach no one.
IMPORT_LINE = f"@{AGENTS_PATH.name}"
...
def _render_rules(shared_rules: str) -> str:
    ...
    return (
        ...
        f"Claude Code reads it through `{CLAUDE_PATH.name}`, which imports it, because Claude Code does\n"
        f"not read `{AGENTS_PATH.name}` itself.\n\n"
        ...
        "## Working Notes\n\n"
        f"{working_notes}\n"
        f"- `{AGENTS_PATH.name}` and `{CLAUDE_PATH.name}` are generated and must not be edited "
        "directly. Edit `docs/agent_rules.md` instead, then run `make refresh-agent-docs`.\n\n"
...
def _render_import_stub() -> str:
    ...
    return (
        "<!--\n"
        f"Generated from `{_display_path(SHARED_RULES_PATH)}` via `scripts/sync_agent_docs.py`. "
        "Do not edit this file manually.\n\n"
        f"Claude Code reads {CLAUDE_PATH.name} and not {AGENTS_PATH.name}, so this file imports the "
        f"one rendered contract instead of\nrepeating it. The import must stay a line of its own, "
        "unindented: inside a code span, inside a fenced\nblock, or under four spaces it is text "
        "rather than an import, and the session then starts with no\nrules at all. See "
        "docs/adr/ADR-011-one-rendered-wrapper.md.\n"
        "-->\n"
        f"{IMPORT_LINE}\n"
    )
...
def build_documents() -> dict[Path, str]:
    ...
    documents = {AGENTS_PATH: _render_rules(shared_rules), CLAUDE_PATH: _render_import_stub()}
