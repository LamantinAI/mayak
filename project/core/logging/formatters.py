...
class NDJSONFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Compute relative source location from record.
        try:
        ...
        # Namespace payload under "data" key to avoid key collisions.
        payload = getattr(record, "payload", None)
        if payload:
            data = safe_serialize(payload)
            if isinstance(data, dict):
                # Promote event_id from data to top-level for fast filtering.
                if "event_id" in data:
                    entry["event_id"] = data.pop("event_id")
                # Promote business-context and tracing fields to top-level
                ...
                # Remove fields already promoted to top-level to avoid duplication.
                for _dup_key in ("span_id", "name", "event_type"):
                    data.pop(_dup_key, None)
                # Strip request-scoped routing fields from non-root events.
                ...
                # Last-resort scrub for free-form text fields built outside
        ...
        # Serialize to single-line JSON via orjson.
