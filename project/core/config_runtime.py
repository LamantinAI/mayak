...
def _sample_placeholder_secrets() -> dict[str, str]:
    ...
    for line in lines:
        ...
        key = key.strip()
        value = value.strip()
        # Skip empty sample values. `.env.sample` leaves the provider key blank,
...
def _declared_app_version() -> str:
    ...
    try:
        ...
        # Never raise. This runs at import time of the settings module, so an
...
class Settings:
    ...
    def validate_runtime(self) -> None:
        ...
        # Reject wildcard CORS in non-debug mode with a membership test, not
        ...
        # Collect what `.env.sample` offers as placeholders, so the checks below
        # reject the sample's own values and not just one hardcoded word.
        # Compared against the sample file's placeholders, not only the literal "postgres":
        ...
        # Reject default PostgreSQL password in non-debug mode — but only when
        ...
        if self.postgres.enabled and not self.project.debug:
            ...
            if password == "postgres" or password == placeholder_secrets.get("POSTGRES_PASSWORD"):
                # The message names the file, because this now fires on a fresh
        ...
        # The same rule for the provider credential. Today .env.sample ships it
        ...
        # Require an API key for any configuration that performs external LLM calls.
        ...
        # Validate prompt directory and default prompt file existence, so a
