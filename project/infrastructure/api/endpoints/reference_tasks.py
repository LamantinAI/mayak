...
async def create_reference_task(
    ...
    # The handler never constructs the service and never touches the repository.
...
async def get_reference_task(
    ...
    # No try/except here. The service raises NotFoundError and
...
async def update_reference_task(
    ...
    # `model_fields_set` is what makes `{"details": null}` mean "clear it" and an
...
async def list_reference_tasks(
    ...
    # Query bounds are declared twice on purpose — here for the OpenAPI contract
