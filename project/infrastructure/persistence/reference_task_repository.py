...
_COLUMNS = "id, title, details, status, created_at"
        ...
        with logger.span("db.reference_task.add", task_id=task.id):
            async with self._pool.connection() as connection:
                await connection.execute(
                    "INSERT INTO reference_tasks (id, title, details, status, created_at) "
                    "VALUES (%s, %s, %s, %s, %s)",
                    (task.id, task.title, task.details, task.status, task.created_at),
                )
